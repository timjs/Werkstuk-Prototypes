...
\end{equation*}
%
De reden voor deze andere aanpak is dat de definitie dan makkelijker leest. We definiëren dat $\Express[B]{b}{\ms,\mo,\sigma,\tau}$ de kleinste subset is van hierboven genoemde verzameling, die aan onderstaande eisen voldoet:
%
\begin{align*}
  &\; \Express[B]{\TRUE}{\ms,\mo,\sigma,\tau} \\
  \neg \Express[B]{b}{\ms,\mo,\sigma,\tau}
  \Rightarrow&\; \Express[B]{\NOT b}{\ms,\mo,\sigma,\tau} \\
  \Express[B]{b}{\ms,\mo,\sigma,\tau} \land \Express[B]{c}{\ms,\mo,\sigma,\tau}
  \Rightarrow&\; \Express[B]{b \AND c}{\ms,\mo,\sigma,\tau} \\
  \Express[B]{b}{\ms,\mo,\sigma,\tau}
  \Rightarrow&\; \Express[B]{b \OR c}{\ms,\mo,\sigma,\tau} \\
...
De kern van de natuurlijke semantiek is een relatie $(\longrightarrow)$, die de eindtoestand na \emph{executie} van een bepaald programma, gegeven een begintoestand en context, bepaalt. De signatuur van deze relatie geeft dit weer:
%
\begin{equation*}
  (\longrightarrow)
  \subseteq
  \underbrace{\Stm}_\text{programma} \times
  \underbrace{(\MMs \times \MMo)}_\text{begintoestand} \times
  \underbrace{(\II \times (\II \cup \{\nil\}))}_\text{context} \times
  \underbrace{(\MMs \times \MMo)}_\text{eindtoestand}
  .
\end{equation*}
%
Een uitspraak
%
\begin{equation*}
  (S,\ms,\mo,\sigma,\tau, \ms',\mo') \in (\longrightarrow)
\end{equation*}
%
zullen we echter ``elegant'' als volgt weergeven, waardoor het intuïtieve concept van executie duidelijk wordt:
%
\begin{equation*}
  \Terminates{S}{\ms,\mo}{\sigma,\tau}{\ms',\mo'}.
\end{equation*}
%
Deze uitspraak moet worden gelezen als: ``Executie van programma $S$, in begintoestand $\ms, \mo$ en met als context het bereik $\sigma$ en huidig object $\tau$, resulteert in eindtoestand $\ms', \mo'$''.
