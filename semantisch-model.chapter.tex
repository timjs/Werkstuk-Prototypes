...
\end{equation*}
We komen later terug op wat de waarden $\VV$ precies zijn in §\ref{sec:waarden}. Voor nu is het voldoende om te weten dat in ieder geval de gehele getallen $\ZZ$ deel uitmaken van $\VV$.
...
\begin{equation*}
  \VV \DEF \LLo \cup \ZZ \cup \FF \cup \{\nil\}
\end{equation*}

\section{Locaties en geheugen}
\label{sec:locaties}

Uit de redenaties over referenties volgt dat we op een bepaalde manier objecten moeten kunnen opslaan aan de hand van deze referenties. Hiervoor gebruiken we een \emph{geheugen voor objecten} $\mo$, een functie met signatuur $\mo : \LLo \to \OO$. De verzameling van alle object geheugens definiëren we als
%
\begin{equation*}
  \MMo \DEF \OO^{\LLo}
\end{equation*}
%
Locaties zijn de semantische tegenhander van referenties. Hun waarde is niet van belang, enkel hun uniciteit die ons in staat stelt objecten te onderscheiden en op te slaan. We nemen aan dat er een functie $\Nexto : \MMo \to \LLo$ bestaat, die aan de volgende eis voldoet:
%
\begin{equation*}
  \forall_{\mo \in \MMo} \left[ \mo \uparrow \Nexto(\mo) \right]
\end{equation*}
%
Ook van bereiken moet een geheugen $\ms : \LLs \to \SS$ worden bijgehouden, op eenzelfde wijze als voor objecten. De verzameling van deze bereik geheugens is op vrijwel identieke manier als volgt gedefiniëerd:
%
\begin{equation*}
  \MMs \DEF \SS^{\LLs}
\end{equation*}
%
Ook nemen we aan dat er een functie $\Nexts : \MMs \to \LLs$ bestaat, die aan de volgende eis voldoet:
%
\begin{equation*}
  \forall_{\ms \in \MMs} \left[ \ms \uparrow \Nexts(\ms) \right]
\end{equation*}
...
Zoals eerder beschreven, moet de definitie van een variabele eerst gezocht worden in het huidige bereik, en daarna in omliggende bereiken. Deze procedure manifesteert zich in de hulpfunctie $\Finds : \MMs \times \LLs \times \Id \to \LLs$, die we als volgt definiëren:
%
\begin{equation*}
  \Finds(\ms, \sigma, i) = \begin{cases}
    \sigma & \textbf{als } b_{\ms(\sigma)}(i) \neq \bot \\
    \bot & \textbf{als } p_{\ms(\sigma)} = \nil \\
    \Finds(\ms, p_{\ms(\sigma)}, i) & \textbf{anders}
  \end{cases}
\end{equation*}
%
Voor het doorzoeken van de prototype hiërarchie naar een attribuut hebben we een hulpfunctie $\Findp : \MMo \times \LLo \times \Id \to \LLo$, die identiek is aan $\Finds$, behalve dat deze werkt met de relevante object-gerelateerde verzamelingen.
%
\begin{equation*}
  \Findp(\mo, \omega, i) = \begin{cases}
    \omega & \textbf{als } b_{\mo(\omega)}(i) \neq \bot \\
    \bot & \textbf{als } p_{\mo(\omega)} = \nil \\
    \Findp(\mo, p_{\mo(\omega)}, i) & \textbf{anders}
  \end{cases}
\end{equation*}
%
Er is ook een hulpfunctie nodig om een pad te doorlopen. De hulpfunctie $\Trav : \MMo \times \LLo \times \Path \to \LLo \times \Id$ levert de locatie van het laatste object op en de laatste \Id\ van het pad (wat mogelijk naar niet noodzakelijk een attribuut is van dat laatste object), gegeven het objectgeheugen, een locatie voor het “begin” object, en een pad.
%
\begin{equation*}
  \Trav(\mo, \omega, p) = \begin{cases}
    (\omega, p) & \textbf{als } p \in \Id \\
    \Trav(\mo, \omega', p') & \textbf{als } p = i.p' \textbf{ en } \Findp(\mo, \omega, i) = \omega' \in \LLo \\
    \bot & \textbf{anders}
  \end{cases}
\end{equation*}
%
