...
In sectie \ref{sec:voorbeelden} is informeel gebleken dat bereiken conceptueel goed te zien zijn als een boomstructuur. Stel dat we een variabele \id{x} in een zeker bereik $s$ willen evalueren tot een waarde. Dan zoeken we \id{x} eerst op in de bindingen groep $b_s$, behorende bij bereik $s$:
%
\begin{equation*}
  b_s(x).
\end{equation*}
...
\begin{enumerate}
  ...
  \item \id{x} is niet gedefinieerd in $b_s$, dus moeten we \id{x} opzoeken in de omliggend bereik.
\end{enumerate}
